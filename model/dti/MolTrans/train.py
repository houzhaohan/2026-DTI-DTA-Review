...
parser.add_argument('--subtask', choices=['full_data', 'unseen_protein', 'unseen_drug'],
                    default='full_data', type=str, metavar='SUBTASK',
                    help='Subtask name for BIOSNAP. Could be full_data, unseen_protein, unseen_drug.')
parser.add_argument('--lr', '--learning-rate', default=1e-4, type=float,
                    metavar='LR', help='initial learning rate', dest='lr')


def get_task(task_name, subtask_name='full_data'):
    if task_name.lower() == 'biosnap':
        return f'./dataset/BIOSNAP/{subtask_name}'
...
def main():
    ...
    dataFolder = get_task(args.task, args.subtask)
    ...
    print('--- Save Predictions to test_p.csv ---')
    try:
        params_pred = {'batch_size': args.batch_size,
                       'shuffle': False,
                       'num_workers': args.workers,
                       'drop_last': False}
        testing_set_pred = BIN_Data_Encoder(df_test.index.values, df_test.Label.values, df_test)
        testing_generator_pred = data.DataLoader(testing_set_pred, **params_pred)

        model_max.eval()
        y_pred_prob = []
        with torch.set_grad_enabled(False):
            for i, (d, p, d_mask, p_mask, label) in enumerate(testing_generator_pred):
                score = model_max(d.long().cuda(), p.long().cuda(), d_mask.long().cuda(), p_mask.long().cuda())
                m = torch.nn.Sigmoid()
                logits = torch.squeeze(m(score), 1)
                logits = logits.detach().cpu().numpy()
                y_pred_prob = y_pred_prob + logits.flatten().tolist()

        assert len(y_pred_prob) == len(df_test), f"Prediction count {len(y_pred_prob)} does not match test samples {len(df_test)}"
        df_test['Prediction'] = y_pred_prob
        test_p_path = dataFolder + '/test_p.csv'
        df_test.to_csv(test_p_path, index=False)
        print(f'Saved predictions to {test_p_path}')
    except Exception as e:
        print(f'Saving predictions failed: {e}')
