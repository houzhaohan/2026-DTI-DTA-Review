...
class DTIDataset(data.Dataset):
    def __init__(self, list_IDs, df, max_drug_nodes=290, smiles_column='SMILES', protein_column='Protein', label_column='Y'):
    ...
    def __getitem__(self, index):
        ...
        v_d = self.df.iloc[index][self.smiles_column]
        v_d = self.fc(smiles=v_d, node_featurizer=self.atom_featurizer, edge_featurizer=self.bond_featurizer)

        num_actual_nodes = v_d.number_of_nodes()
        if num_actual_nodes > self.max_drug_nodes:
            v_d = v_d.subgraph(range(self.max_drug_nodes))
            for key in list(v_d.ndata.keys()):
                if '_ID' in key:
                    del v_d.ndata[key]
            for key in list(v_d.edata.keys()):
                if '_ID' in key:
                    del v_d.edata[key]
        ...
        v_p = self.df.iloc[index][self.protein_column]
        v_p = integer_label_protein(v_p)
        y = self.df.iloc[index][self.label_column]
        return v_d, v_p, y
