...
class Trainer(object):
    def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader, opt_da=None, discriminator=None,
        ...
        self.best_test_auroc = 0

        self.train_loss_epoch = []
        self.train_model_loss_epoch = []
        self.train_da_loss_epoch = []
        self.val_loss_epoch, self.val_auroc_epoch = [], []
        self.test_loss_epoch, self.test_auroc_epoch = [], []
        self.test_metrics = {}
        self.config = config
    ...
    def predict_proba(self, dataloader="test"):
        if dataloader == "test":
        ...
        y_pred_probs = []
        model = self.best_test_model if dataloader == "test" else self.model
        with torch.no_grad():
            model.eval()
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            test_auroc, test_auprc, _, _, _, _, test_loss, _, _ = self.test(dataloader="test", model=self.model)
            self.test_loss_epoch.append(test_loss)
            self.test_auroc_epoch.append(test_auroc)
            if self.experiment:
                self.experiment.log_metric("test_epoch model loss", test_loss, epoch=self.current_epoch)
                self.experiment.log_metric("test_epoch auroc", test_auroc, epoch=self.current_epoch)
                self.experiment.log_metric("test_epoch auprc", test_auprc, epoch=self.current_epoch)
            if test_auroc >= self.best_test_auroc:
                self.best_test_model = copy.deepcopy(self.model)
                self.best_test_auroc = test_auroc
                self.best_test_epoch = self.current_epoch
            print('Test at Epoch ' + str(self.current_epoch) + ' with test loss ' + str(test_loss), " AUROC "
                  + str(test_auroc) + " AUPRC " + str(test_auprc))

        auroc, auprc, f1, sensitivity, specificity, accuracy, test_loss, thred_optim, precision = self.test(dataloader="test")
        test_lst = ["epoch " + str(self.best_test_epoch)] + list(map(float2str, [auroc, auprc, f1, sensitivity, specificity,
                                                                            accuracy, thred_optim, test_loss]))
        self.test_table.add_row(test_lst)
        print('Test at Best Test Model of Epoch ' + str(self.best_test_epoch) + ' with test loss ' + str(test_loss), " AUROC "
        ...
        self.test_metrics["best_epoch"] = self.best_test_epoch
        self.test_metrics["best_val_epoch"] = self.best_epoch
        ...
        if self.experiment:
            ...
            self.experiment.log_metric("test_best_auroc", self.best_test_auroc)
            self.experiment.log_metric("test_best_epoch", self.best_test_epoch)
    ...
    def save_result(self):
        if self.config["RESULT"]["SAVE_MODEL"]:
            ...
            torch.save(self.best_test_model.state_dict(),
                       os.path.join(self.output_dir, f"best_test_model_epoch_{self.best_test_epoch}.pth"))
        ...
        state = {
            ...
            "test_epoch_loss": self.test_loss_epoch,
            "test_epoch_auroc": self.test_auroc_epoch,
    ...
    def test(self, dataloader="test", model=None):
        test_loss = 0
        y_label, y_pred = [], []
        if dataloader == "test":
            data_loader = self.test_dataloader
