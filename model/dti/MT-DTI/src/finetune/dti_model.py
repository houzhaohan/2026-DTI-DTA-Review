...
class MbertPcnnModel(object):
    def __init__(self, batch_size, dev_batch_size, max_molecule_length, max_protein_length,
                 ...
                 use_tpu, kernel_size1, kernel_size2, kernel_size3, task_type='regression'):
        self.batch_size = batch_size
    ...
    def _compute_loss_and_predictions(self, z, y):
        """
        Compute predictions and loss based on task type.
        For regression: linear output + MSE loss
        For classification: sigmoid output + binary cross-entropy
        """
        if self.task_type == 'classification':
            logits = tf.layers.dense(z, 1, kernel_initializer='normal')
    ...
    def _get_eval_metric_fn(self):
        """Return a metric function appropriate for the task type."""
        if self.task_type == 'classification':
            def metric_fn_classification(loss, y_true, y_pred):
                mean_loss = tf.metrics.mean(values=loss)
                # y_pred is already sigmoid output [batch, 1], squeeze for metrics
                y_pred_flat = tf.squeeze(y_pred, axis=-1)
                y_true_flat = tf.squeeze(y_true, axis=-1)
                # AUC
                auc = tf.metrics.auc(labels=y_true_flat, predictions=y_pred_flat)
                # Accuracy
                acc = tf.metrics.accuracy(labels=y_true_flat,
                                          predictions=tf.cast(y_pred_flat > 0.5, tf.float32))
                return {
                    "loss": mean_loss,
                    "auc": auc,
                    "accuracy": acc,
                }
            return metric_fn_classification
        else:
            def metric_fn_regression(loss, y_true, y_pred):
                mean_loss = tf.metrics.mean(values=loss)
    ...
    def model_fn_v1(self, features, labels, mode, params):
        ...
        predictions, loss = self._compute_loss_and_predictions(z, y)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
            ...
            eval_metric_fn = self._get_eval_metric_fn()
            eval_metrics = (eval_metric_fn, [loss, y, predictions])
