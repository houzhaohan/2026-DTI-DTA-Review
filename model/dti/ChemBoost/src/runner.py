...
from src.utils import parse_terminal_arguments, get_repr_model, dict_cartesian_product, load_biosnap_data
...
if is_biosnap:
    print('BIOSNAP dataset detected — loading data dynamically')
    train_folds, test, ligands, proteins, sb_threshold, raw_test_path = load_biosnap_data(dataset, configs)
    ...
    representation_model = get_repr_model(dataset, model, configs,
                                          ligands=ligands, proteins=proteins)
else:
    ...
    for fold in train_folds:
        fold['ligand_id'] = fold['ligand_id'].astype(str)
    raw_test_path = configs[f'{dataset}_test']
...
mean_predictions = test_models(best_models, representation_model, train_folds, test, sb_threshold, model_save_path)
print('DONE!')

# Save test_r.csv: original test.csv + Prediction column
if raw_test_path is not None:
    raw_test_df = pd.read_csv(raw_test_path)
    raw_test_df['Prediction'] = mean_predictions
    test_r_path = os.path.join(model_save_path, 'test_r.csv')
    raw_test_df.to_csv(test_r_path, index=False)
    print(f'Saved test_r.csv → {test_r_path}')
