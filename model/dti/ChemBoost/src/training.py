...
def test_models(xgb_models, representation_model, folds, test, sb_threshold, save_path):
    ...
    pd.DataFrame(all_test_predictions).T.to_csv(save_path + 'test_predictions.csv', index=None)

    # Return mean predictions across folds for downstream use (e.g. test_r.csv)
