...
from ..dataset.datamodules import drug_target_collate_fn
...
def predict_probs(model, data_generator, device=None):
    if device is None:
        device = torch.device("cpu")
    ...
    with torch.set_grad_enabled(False):
        for _, batch in tqdm(enumerate(data_generator), total=len(data_generator)):
            pred, _ = step(model, batch, device)
            preds.append(pred.detach().cpu().numpy())

    if len(preds) == 0:
        return np.array([])

    preds = np.concatenate([p.reshape(-1) for p in preds], axis=0)
    return preds
    ...
    try:
        with torch.set_grad_enabled(False):
            ...
            try:
                ...
                test_probs = predict_probs(model_max, predict_loader, device)
                df_test = datamodule.df_test.copy()
                df_test["Prediction"] = test_probs

                invalid_drugs = set()
                missing_proteins = set()

                def collect_invalid(featurizer, attr_name, result_set):
                    if hasattr(featurizer, attr_name):
                        result_set.update(getattr(featurizer, attr_name))
                    elif hasattr(featurizer, '_cuda_registry'):
                        for name in getattr(featurizer, '_featurizer_names', []):
                            f_instance = featurizer._cuda_registry[name][0]
                            collect_invalid(f_instance, attr_name, result_set)

                collect_invalid(drug_featurizer, '_invalid_smiles', invalid_drugs)
                collect_invalid(target_featurizer, 'missing_proteins', missing_proteins)

                drug_col = datamodule._drug_column
                target_col = datamodule._target_column

                for idx, row in df_test.iterrows():
                    drug = row[drug_col]
                    target = row[target_col]
                    if drug in invalid_drugs or target in missing_proteins:
                        df_test.at[idx, 'Prediction'] = np.nan

                test_pred_path = task_dir / "test_p.csv"
                df_test.to_csv(test_pred_path, index=False)
                logg.info(f"Saved test predictions to {test_pred_path}")
                logg.info(f"Marked {len(invalid_drugs)} invalid drugs and {len(missing_proteins)} missing proteins as NA")
            except Exception as e:
                logg.error(f"Failed to write test_p.csv: {e}")
