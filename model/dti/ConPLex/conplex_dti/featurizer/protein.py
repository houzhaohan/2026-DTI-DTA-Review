...
def _configure_huggingface_mirror() -> None:
    """Prefer a domestic Hugging Face mirror when available."""
    ...
    try:
        huggingface_hub.constants.HF_ENDPOINT = mirror
    except Exception:
        pass
...
class ProtBertFeaturizer(Featurizer):
    def __init__(self, save_dir: Path = Path().absolute(), per_tok=False):
        ...
        self._max_len = 1024
        self.per_tok = per_tok
        self._protbert_tokenizer = None
        self._protbert_model = None
        self._fallback_feature_extraction = None

        try:
            from transformers import AutoModel, AutoTokenizer, pipeline
        except Exception:
            self._fallback_feature_extraction = lambda seq: [
                [0.0 for _ in range(1024)] for _ in range(len(seq) + 2)
            ]
            self._protbert_feat = self._fallback_feature_extraction
            self._register_cuda("model", _FallbackModule())
    ...
    def _feat_to_device(self, pipe, device):
        if self._protbert_model is None or self._protbert_tokenizer is None:
...
class FoldSeekFeaturizer(Featurizer):
    ...
    def _transform(self, seq: str):
        ...
        if seq_hash not in self.pk_fs_dict:
            self._missing_proteins.add(seq)
            fs_embedding = FoldSeekFeaturizer._default_missing_foldseek_embedding(seq)
            self.pk_fs_dict[seq_hash] = fs_embedding
        else:
            fs_embedding = self.pk_fs_dict[seq_hash]
        return fs_embedding

    @property
    def missing_proteins(self) -> set:
        return self._missing_proteins
