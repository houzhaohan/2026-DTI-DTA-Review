...
def extract_dti() -> None:
    # biosnap dataset: read from pre-split CSVs in subfolders
    dataset = "biosnap"
    data_path = "../data/dti/" + dataset + "/"
    save_path = data_path + "features/"
    os.makedirs(save_path, exist_ok=True)

    subfolders = ["full_data", "unseen_drug", "unseen_protein"]
    all_dfs = []
    for sub in subfolders:
        for split in ["train.csv", "val.csv", "test.csv"]:
            csv_path = os.path.join(data_path, sub, split)
            if os.path.exists(csv_path):
                df = pd.read_csv(csv_path)
                all_dfs.append(df)

    biosnap_data = pd.concat(all_dfs, ignore_index=True)
    biosnap_data = biosnap_data[["DrugBank ID", "Gene", "SMILES", "Target Sequence"]].drop_duplicates()
    biosnap_data.columns = ["cid", "pid", "smi", "seq"]

    drug_smi = biosnap_data[["cid", "smi"]].drop_duplicates()
    tar_seq = biosnap_data[["pid", "seq"]].drop_duplicates()
