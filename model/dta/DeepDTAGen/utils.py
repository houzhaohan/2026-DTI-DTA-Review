...
class TestbedDataset(InMemoryDataset):
    ...
    def process(self, xd, xdt, xt, y, smile_graph):
        ...
        #data_list = []  # 初始化存储有效数据的列表
    
        # 第一步：遍历所有数据，过滤掉xdt[i]长度>94的样本
        valid_xdt = []  # 存储过滤后的有效xdt序列
        valid_xd = []   # 对应过滤后的xd
        valid_xt = []   # 对应过滤后的xt
        valid_y = []    # 对应过滤后的y
        ...
        for i in range(data_len):
            ...
            current_xdt = xdt[i]  # 当前xdt序列
        
            # 核心过滤：如果xdt长度>94，直接跳过该样本
            if len(current_xdt) > 94:
                continue
        
            # 长度≤94，保留该样本的所有数据
            valid_xdt.append(current_xdt)
            valid_xd.append(xd[i])
            valid_xt.append(xt[i])
            valid_y.append(y[i])
...
def max_min(lst):
    if not lst:  # 如果二维列表为空
        return (0, 0)  # 或者可以返回 (None, None) 或根据需求处理
