...
class PMMRNet(torch.nn.Module):
    def __init__(self, args):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=self.compound_text_dim, dim_feedforward=self.feedforward_dim, nhead=self.encoder_heads, batch_first=True)
        self.transformer_encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=self.encoder_layers)

        self.encoder_layer2 = nn.TransformerEncoderLayer(d_model=self.protein_dim, dim_feedforward=self.feedforward_dim, nhead=self.encoder_heads, batch_first=True)
    ...
    def forward(self, data):
        ...
        # SMILES padding mask: True 表示 padding 位置
        B, Ls, _ = compound_emb.shape
        smiles_len = data["COMPOUND_SMILES_LENGTH"].to(device)
        smiles_padding_mask = torch.arange(Ls, device=device)[None, :] >= smiles_len[:, None]
        # add yiheng

        # drug_mask = self.generate_attention_mask(256,4,data['COMPOUND_SMILES_LENGTH'])
        compound_smiles = self.transformer_encoder(self.fc3(compound_emb), src_key_padding_mask=smiles_padding_mask)
        ...
        # add yiheng
        B, Lt, _ = target_emb.shape
        protein_len = data["PROTEIN_NODE_NUM"].to(device)
        protein_padding_mask = torch.arange(Lt, device=device)[None, :] >= protein_len[:, None]
        # add yiheng

        #Protein
        # target_mask = self.generate_attention_mask(256, 4, data["PROTEIN_NODE_NUM"])
        seq_emb = self.transformer_encoder2(self.fc2(target_emb), src_key_padding_mask=protein_padding_mask)
