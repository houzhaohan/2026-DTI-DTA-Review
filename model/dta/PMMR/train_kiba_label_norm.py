...
def train(model, device, train_loader, optimizer, mse_f, epoch, train_data, FLAGS, y_mean, y_std):
    model.train()
    ...
    with tqdm(train_loader, desc=f"Epoch {epoch + 1}") as t:
        for i, data in enumerate(t):
            ...
            y_raw = data.y.view(-1, 1).float().to(device)
            y_norm = (y_raw - y_mean) / y_std
            mse_loss = mse_f(Pridection, y_norm)

            # train_ci = get_cindex(Pridection.cpu().detach().numpy(), data.y.view(-1, 1).float().cpu().detach().numpy())

            loss = kl_loss * 0.001 + lm_loss
            # loss.backward()
            # optimizer.step()
            ...
            optimizer.step()
            t.set_postfix(MSE=mse_loss.item(), Train_cindex=0, KL=kl_loss.item(), LM=lm_loss.item())
        msg = f"Epoch {epoch + 1}, total loss={loss.item()}, MSE={mse_loss.item()}, KL_loss={kl_loss.item()}, LM={lm_loss.item()}"
        logging(msg, FLAGS)
    return model


def val(model, device, val_loader, dataset, FLAGS, y_mean, y_std):
    ...
    P = P * y_std.item() + y_mean.item()
    mse_loss = mse(G, P)
    val_ci = 0
    rm2 = 0

    # val_ci = get_cindex(G, P)
    # rm2 = get_rm2(G, P)
    # auc_values = []
    # for t in thresholds:
    #    auc = get_aupr(G, P, t)
...
def test(model, device, test_loader, dataset, FLAGS, y_mean, y_std):
...
def experiment(FLAGS, dataset, device, roundid):
    ...
    if not (os.path.isfile(processed_data_file_train) and os.path.isfile(processed_data_file_test)):
        ...
        # Label normalization: only use training set statistics
        train_y = train_data.data.y.float()
        y_mean = train_y.mean().to(device)
        y_std = train_y.std(unbiased=False).to(device)
        if y_std < 1e-8:
            raise ValueError("Training label std is too small, cannot normalize labels.")
        print(f"Train label mean: {y_mean.item():.6f}")
        print(f"Train label std: {y_std.item():.6f}")
        ...
        for epoch in range(NUM_EPOCHS):
            model = train(model, device, train_loader, optimizer, mse_f, epoch, train_data, FLAGS, y_mean, y_std)

            # 2. 验证
            val_total_loss, val_mse_loss, val_ci, val_rm2, _, _ = val(model, device, val_loader, dataset, FLAGS, y_mean,
                                                                      y_std)
            ...
            result_dir_lxc = f'res_kiba/{dataset}/round_{roundid}'
    ...
    # 加载最佳模型
    model.load_state_dict(torch.load(f"saved_models/deepdtagen_model_{dataset}_kiba_{roundid}_best.pth"))
    model.eval()

    test_total_loss, test_mse_loss, test_ci, test_rm2, G, P = test(model, device, test_loader, dataset, FLAGS, y_mean,
                                                                   y_std)
    result_dir = f"./results/{dataset}/round_{roundid}/"
    ...
    record_file = f"./results/{dataset}/record_{roundid}"
