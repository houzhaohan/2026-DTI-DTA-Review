...
import random
# InfoNCE 未实际使用，移除依赖以避免 ModuleNotFoundError
...
class Encoder(nn.Module):
    def __init__(self, num_features, dim, num_gc_layers, times, mode):
        ...
        self.mode = mode
        self.nce = None  # 原 self.nce = InfoNCE() 未实际使用
    ...
    def forward(self, x, edge_index, batch, percent):
        ...
        if percent:
            ...
            # 替换n（纯 GPU 向量化，消除 CPU-GPU 同步瓶颈）
            num_sub = self.num_subgraph
            N_graphs = int(batch[-1]) + 1

            # 与原逻辑等价：零掉前 num_sub 列，排除特定子图组的匹配
            cos[:, :num_sub] = 0

            # 每行的最大相似度及其 argmax（全程 GPU）
            max_vals, max_idx = cos.max(dim=-1)                          # (N*num_sub,)

            # 按 graph 分组
            max_vals_g = max_vals.view(N_graphs, num_sub)                # (N, num_sub)
            max_idx_g  = max_idx.view(N_graphs, num_sub)                 # (N, num_sub)

            # 组内按相似度降序排序
            _, sorted_order = max_vals_g.sort(dim=-1, descending=True)   # (N, num_sub)

            # 取前一半（相似度最高的 top_k 个子图）
            top_k = num_sub // 2                                         # 2
            top_local_idx = sorted_order[:, :top_k]                      # (N, 2)

            # 对应的 argmax 目标（全局索引）
            key_global = max_idx_g.gather(1, top_local_idx)               # (N, 2)

            # 计算写入位置的全局索引
            group_offsets = torch.arange(N_graphs, device=sub.device) * num_sub
            dst_global = top_local_idx + group_offsets[:, None]           # (N, 2)

            # 展平后一次性替换（先提取 src 再写入 dst，避免原地冲突）
            key_flat = key_global.reshape(-1)                             # (N*2,)
            dst_flat = dst_global.reshape(-1)                             # (N*2,)
            src = sub[key_flat]                                           # (N*2, dim)
            sub[dst_flat] = src

            sub_graph = sub.reshape(N_graphs, num_sub, self.dim).unsqueeze(-1)
