...
parser.add_argument('-d', '--dataset', type=str, default='kiba',
                    help='Dataset name (default: davis, support kiba etc.)')

parser.add_argument('-r', '--roundid', type=int, default=0)
...
# 模型保存路径
model_save_path = f'./Model/{dataset_name}_best_model_{roundid}.pt'
...
def reset_feature_1(dataset, model):
    """重置数据集特征"""
...
def reset_feature(dataset, model):
    """重置数据集特征 - 计算后立刻移CPU，不占GPU显存"""
    torch.cuda.empty_cache()
    batch_size = 128   # 嫌爆显存再改成32/16
    with torch.no_grad():
        model.eval()
        dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
        start = 0
        for data in tqdm(dataloader, desc="Resetting features"):
            ...
            tar_len_list = []
            idx_list = []
            cur_bs = min(batch_size, len(dataset) - start)
            ...
            # 更新数据集特征：全程CPU运算
            for i in range(start, start + cur_bs):
                batch_idx = i - start
                t_len = tar_len_list[batch_idx]
                s_idx = idx_list[batch_idx]
                total_len = t_len + len(s_idx) + 1

                # 2. source 直接在CPU创建，不to(device)
                source = torch.zeros(total_len, dtype=torch.long)
                source[:t_len] = 0
                source[t_len : t_len + len(s_idx)] = 1
                source[-1] = 2

                # 3. 特征拼接全部在CPU
                feat1 = pro[batch_idx, 1 : t_len + 1]
                feat2 = sm[batch_idx, s_idx]
                feat3 = (sm[batch_idx, 0] + pro[batch_idx, 0]) / 2
                feat3 = feat3.unsqueeze(0)

                feature = torch.cat([feat1, feat2, feat3], dim=0)
                new_feature = torch.cat([feature, source.unsqueeze(-1)], dim=-1)
...
for epoch in range(NUM_EPOCHS):
    ...
    result_dir_lxc = f'res/{dataset_name}/round_{roundid}'
...
result_dir = f"./results/{dataset_name}/round{roundid}/"
...
mse_loss = get_mse(G, P)
concordance_index = get_cindex(G, P)
rm2_value = get_rm2(G, P)


record_file = f"./results/{dataset_name}/record{roundid}"
